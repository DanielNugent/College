...
int getBonus(int avgSleep)
{
  if (avgSleep >= 1000)
    return 10;
  return avgSleep / 100;
}
void updateAvgSleep(struct proc *p, int time)
{
  p->totalsleep += time;
  p->timesslept += 1; 
  if (p->timesslept != 0)
    p->avgsleep = p->totalsleep / p->timesslept;
}
...
void initproc(struct proc *p)
{
  ...
  p->timesslept = 0; //amount of times asleep
  p->totalsleep = 0;  //total time asleep
}

void adjustpriority(struct proc *p)
{
  int sleepTime = clock - p->lastrun;
  updateAvgSleep(p, sleepTime);
  int dynamicPrio;
  int minVar1 = p->staticprio - getBonus(p->avgsleep) + 5;
  if (minVar1 <= 139)
  {
    dynamicPrio = minVar1;
  }
  ...
  p->dynprio = dynamicPrio;

  int baseQuantum;
  if (p->staticprio < 120)
  {
    baseQuantum = (140 - p->staticprio) * 20;
  }
  else
  {
    baseQuantum = (140 - p->staticprio) * 5;
  }
  p->quantum = baseQuantum;
}
